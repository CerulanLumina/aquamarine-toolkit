use aquamarine_toolkit_api::{AquamarineMessage, PluginDefinition};
...
impl AquamarineServer {
    pub fn new(addr: SocketAddr) -> Result<Self, Box<dyn Error>> {
        ...
        socket
            .map(|a| AquamarineServer { udp_socket: a })
            .map_err(|a| {
    ...
    pub fn run_server(self, plugins_map: HashMap<String, PluginDefinition>) -> ! {
...
pub fn start_server(addr: SocketAddr) {
    ...
    match AquamarineServer::new(addr) {
        Ok(server) => server.run_server(map),
        Err(err) => {
